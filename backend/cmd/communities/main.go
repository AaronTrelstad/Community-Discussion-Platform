...
func main() {
	...
	queries := db.New(conn)
	communityService := services.NewCommunityService(queries)
	communitiesHandler := handlers.NewCommunitiesHandler(communityService)
	...
	r.Get("/communities", communitiesHandler.ListCommunities)

	r.Group(func(r chi.Router) {
		r.Use(internalmiddleware.Auth)
		r.Post("/communities", communitiesHandler.CreateCommunity)
		r.Put("/communities/{id}", communitiesHandler.UpdateCommunity)
	})

	log.Println("Running locally on :8081")
